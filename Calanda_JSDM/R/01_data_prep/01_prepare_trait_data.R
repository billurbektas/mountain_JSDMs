...
nc_data_13 <- clean_nc_data("data/traits/N+C_Samples13.xls")

# Preview the cleaned data
print(nc_data_13)

# Save cleaned data
write_csv(nc_data_13, "output/nc_samples_13_cleaned.csv")
...
nc_files <- c(list.files("data/traits",
                       pattern = "N\\+C_Samples.*\\.xls$",
                       full.names = TRUE),
              list.files("data/traits",
                         pattern = "Trial",
                         full.names = TRUE)
             )
...
sample_id_mapping <- read_csv("data/traits/2025_CAPHE_traits_sample_id.csv")
...
outlier_c =
  all_nc_data %>%
  filter(LCC>70|LCC<20)%>% # Above/below these numbers probably measurment error.
  pull(cn_ID)
...
# Save combined data
write_csv(nutrients, "output/leaf_nutrients_cleaned.csv")
...
traits_clean <- read_csv("data/traits/2025_TRAITS_clean_20251023.csv")
...
# Read leaf_area data
leaf_area <- read_csv("data/traits/leaf_area.csv")
...
# Save all datasets
write_csv(all_traits, "output/all_traits_merged.csv")
write_csv(final_traits, "output/final_traits_for_analysis.csv")
write_csv(final_traits_clean, "output/final_traits_clean.csv")
...
write_csv(species_traits, "output/species_trait_summary.csv")
...
pdf("plot/corrplot_all_traits.pdf", width = 14, height = 14)
corrplot(cor_all,
         method = "color",
         type = "upper",
         order = "hclust",
...
pdf("plot/pca_trait_means_biplot.pdf", width = 12, height = 10)
p1 <- ggplot() +
  # Add variable arrows
...
pdf("plot/pca_kcv_biplot.pdf", width = 12, height = 10)
p2 <- ggplot() +
  # Add variable arrows
...
# Eigenvalues
write_csv(as.data.frame(pca_means$eig), "output/pca_means_eigenvalues.csv")
write_csv(as.data.frame(pca_kcv$eig), "output/pca_kcv_eigenvalues.csv")

# Variable contributions
write_csv(as.data.frame(pca_means$var$contrib), "output/pca_means_var_contrib.csv")
write_csv(as.data.frame(pca_kcv$var$contrib), "output/pca_kcv_var_contrib.csv")
