# ==============================================================================
# Script: imputation_and_coverage_assessment.R (archived from 03_merge_and_assess_traits.R)
# Purpose: Trait imputation and coverage bias assessment
#
# Description:
#   This code was extracted from 03_merge_and_assess_traits.R.
#   It contains:
#     1. Trait imputation using random forest (impute_functional_traits)
#     2. Coverage assessment (Part B): per-plot trait coverage, bias testing
#        against JSDM variance components at community and species levels.
#
# Prerequisites:
#   Run 03_merge_and_assess_traits.R first to produce:
#     - output/traits_raw.csv (or traits_ordered object in memory)
#     - output/all_traits_individual.csv (or all_individual object in memory)
#
# ==============================================================================
...
# Load raw traits if not already in memory
if (!exists("traits_ordered")) {
  traits_ordered = read_csv(here("Calanda_JSDM", "output", "traits_raw.csv"),
...
traits_imputed = imp_traits$imputed_data %>%
  select(species_TNRS, contains("_final")) %>%
...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║  ASSESS TRAIT COVERAGE AND BIAS                                          ║
# ║  (requires starter_data_25.04.25.RData from a prior run of step 04)     ║
# ╚════════════════════════════════════════════════════════════════════════════╝
...
if (!file.exists(starter_data_path)) {
  ...
  cat("starter_data_25.04.25.RData not found (run 04_prepare_climate_vegetation_data.R first).\n")
...
# Load individual trait data if not in memory
if (!exists("all_individual")) {
  all_individual = read_csv(here("Calanda_JSDM", "output", "all_traits_individual.csv"),
                            show_col_types = FALSE)
}

# Get list of species with trait data
